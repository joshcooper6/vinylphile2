...
export default function AlbumCard({vinyl, setCart, cart}) {
    ...
    function addToCart(album) {
        ...
        if ((!existingAlbum) && (album.inventory > 0)) { setCart(prev => [...prev, album])  };
        ...
        if (album.inventory <= 0) {
         ...
         setCart(prev => {
            return prev.map(vinyl => {
                if ((vinyl.id === album.id) && (vinyl.inventory > vinyl.quantity)) {
    ...
    return <div className="flex w-11/12 md:w-8/12 border-[1px] p-6 max-w-[400px] justify-between items-center">
        <div onClick={() => { vinyl.inventory > 0 ? addToCart(vinyl) : null }} className={`relative ${vinyl.inventory <= 0 ? 'disabled' : 'cursor-pointer' } flex`} onMouseLeave={() => setVinylShow(false)} onMouseOver={() => setVinylShow(true)}  >
            <div 
                className={`bg-gray-600 ${vinyl.inventory <= 0 ? 'hidden' : ''} absolute transease ${vinylShow ? 'translate-x-[-50px]' : 'translate-x-0'} z-0 self-center rounded-tl-full rounded-bl-full h-[150px] w-[100px]`}
                children={' '} 
            />
            <img 
                src={vinyl.albumCover} 
                className={'max-w-[150px] bg-blue-800 shadow-lg z-10 rounded-lg object-cover'}
            />
            <div 
                children={<span>Check<br />back<br />soon</span>}
                className={`${vinyl.inventory <= 0 ? '': 'hidden'} w-[150px] p-2 text-center font-black uppercase text-xl h-[150px] flex flex-col items-center justify-center bg-white z-20 rounded-lg opacity-80 absolute`}
            />
        </div>
